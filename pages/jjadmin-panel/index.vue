...
<template>
  ...
  >
    <div
      class="w-full px-2 sm:w-[80%] lg:w-[60%] flex items-center justify-between mb-8"
    >
      <NuxtLink
        to="/"
        class="w-10 md:min-w-[8rem] lg:min-w-[10rem] h-10 md:h-12 bg-[#eee] rounded-lg hover:cursor-pointer hover:bg-[#ccc] active:bg-[#ccc] flex items-center justify-center gap-2"
      >
        <i class="pi pi-home"></i>
        <p class="hidden md:flex">Strona główna</p>
      </NuxtLink>
      <p class="text-[#444] text-2xl font-bold text-center">
      ...
      >
        ...
        <p class="hidden md:flex">Wloguj</p>
      </button>
      <button
        @click="handleLogout"
        v-if="loading"
        class="w-10 md:min-w-[8rem] lg:min-w-[10rem] h-10 md:h-12 bg-[#eee]/70 rounded-lg"
      >
