...
const { loading, editCustomElement } = useContent();
...
const handleElementEdit = async () => {
  ...
  await editCustomElement(newEditData);

  emit("refresh");
  emit("close");
};

const onSubmit = handleSubmit(handleElementEdit);
</script>
...
<template>
  ...
  >
    <form
      ...
      class="w-[90%] max-w-[24rem] flex flex-col items-center p-4 gap-4 bg-[#D9D9D9] rounded-xl pb-8 z-40"
    >
